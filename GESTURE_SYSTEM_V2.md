...
| `src/gesture/SingleHandZoom.ts`                                                                                                                                           | 五指撮合保持启动、连续开合缩放、张掌停留结束与当前比例保留；独立占用输入。                                  |
...
先把五个指尖向前聚拢成撮合姿势，识别为 `FIVE_PINCH`，可靠保持 **180ms** 后启动缩放。随后聚拢缩小、张开放大；五指开合幅度映射到 **0.35–1.75 倍**，不使用双掌距离或手在屏幕上的移动量。手完全张开并停留 **450ms**，或移出画面，结束缩放并保留大小。回到中间开合程度后可继续调节，无需用两只手。

`gripAperture` 是五个指尖相对其中心的 RMS 离散程度，并按掌宽归一化，因此摄像头前后距离本身不直接控制缩放；`gripConfidence` 是五指几何适用于开合控制的可信度。拇指与食指单独接触仍是选择／拖动；四指卷进掌心仍是握拳返回。缩放确认和进行期间屏蔽这些其他命令，避免收拢动作切入错误模式。原 `TWO_HAND_ZOOM` 动作移除，由 `ONE_HAND_ZOOM` 替代；双手仍用于坍缩与重生彩蛋。
...
以下逐项列出两份配置文件的全部 **113 个字段（71 + 42）**。`ms` 是毫秒，`s` 是秒；屏幕坐标为镜像后的 0–1 归一化坐标，二维距离不是像素。掌宽比优先使用世界关键点的三维距离除以三维掌宽；缺少有效世界关键点时使用宽高比校正后的图像关键点。指数响应采用 `1 - exp(-dt × response)`，时间常数采用 `1 - exp(-dt / time)`，两者调节方向相反。
...
| `ONE_HAND_ZOOM_HOLD_TIME`         |    180 | ms             | 五指撮合连续可靠保持后启动开合缩放的时间。                                                              |
| `ONE_HAND_ZOOM_MIN_CONFIDENCE`    |   0.65 | 0–1            | 五指开合几何可信度门槛；启动还必须识别为 FIVE_PINCH。                                                   |
...
### `src/gesture/recognizerConfig.ts`（42 项）
...
| `GRIP_CENTER_FORWARD_MIN`        |   0.25 | 掌宽比                 | 指尖共同中心相对四指根中心、沿手腕至指根方向的前伸投影下限；不要求朝屏幕特定方向。           |
| `GRIP_CENTER_TO_PALM_MIN`        |    0.8 | 掌宽比                 | 指尖共同中心与掌心的距离下限，区分向外撮合和卷进掌心的拳头。                                 |
| `GRIP_FINGER_WRIST_MIN`          |   0.94 | 距离比                 | 四指中最低的指尖至腕／PIP 至腕距离比门槛，防止个别手指收进掌心。                             |
| `GRIP_FINGER_WRIST_STRONG`       |   1.02 | 距离比                 | 一根手指参与开合几何时所需的较强离腕比门槛，与参与手指数联合判断。                           |
| `GRIP_FINGER_COUNT`              |      3 | 根                     | 四指中至少达到较强离腕比的手指数。                                                           |
...
|      2 | 单手 `ONE_HAND_ZOOM`               | 五指撮合保持后，以开合控制；确认及运行期间禁止同时选择、拖动、返回或拨动。         |
...
面板包含左右手与数量、`Pose Confidence`、`Hand Geometry: VALID / WEAK`、当前几何姿势、仲裁后的 Action、Pinch 阶段／距离、五指缩放状态／启动进度、平滑归一化开度和当前倍数、掌心位置、五指状态、Gesture State、App State、重入就绪状态、Gesture Lock、是否必须松手、剩余冷却、目标、三个确认进度、`Special Stage` 和渲染 FPS。姿势分数与有效几何分开，避免把「看见手」与「动作确认」混为一谈。
...
8. **五指缩放难启动或响应不适合设备**：先看 Debug 的 `Current Gesture`、`Five-Finger Zoom` 和 `Zoom Aperture`，确认指尖向前聚拢而非卷进掌心。代码侧可进一步检查原始 `gripAperture`／`gripConfidence`；这两个原始字段未直接显示在当前 Debug 面板中。行为层先调 `ONE_HAND_ZOOM_HOLD_TIME`、`ONE_HAND_ZOOM_CLOSED_APERTURE`、`ONE_HAND_ZOOM_OPEN_APERTURE` 与 `ONE_HAND_ZOOM_SMOOTHING_TIME`；需要调整张掌结束时间则改 `ONE_HAND_ZOOM_RELEASE_HOLD_TIME`。`ZOOM_MIN`、`ZOOM_MAX`、`ZOOM_RESPONSE` 控制范围与渲染响应。鼠标滚轮单独调 `WHEEL_ZOOM_SENSITIVITY`。
...
11. **五指撮合被当成握拳／双指捏合**：观察镜像骨架并复核原始五指尖分布，再调整 `FIVE_PINCH_APERTURE_MAX`、`FIVE_PINCH_RADIUS_MAX` 和 `GRIP_*` 几何门槛；每次都同时复核普通握拳、双指捏合和单根手指未聚拢的反例。
