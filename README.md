...
## Gesture System V2：指、捏、拖、拨、握

等待开场结束，点击 **开启手势控制** 并允许摄像头访问。让手完整出现在镜头内，保持光线充足。摄像头画面默认隐藏，手重新出现后的前 **250ms** 只跟踪光标，随后显示就绪。
...
- **宇宙彩蛋**：在太阳系总览中双掌张开，缓慢向中心靠近约 1 秒，触发坍缩。等待能量核心形成后，再将双掌从中心向两侧拉开，触发重生与粒子爆发，太阳系重新展开。此动作不进入太阳。
...
打开 [手势调试模式](https://alzat007.github.io/solaris/?debugGesture=true)，或在本地地址后加 `?debugGesture=true`，查看镜像骨架、左右手、置信度、五指状态、Pinch 阶段、目标、场景、重入锁、冷却和渲染 FPS。正式访问默认隐藏调试信息。再次点击摄像头按钮即停止追踪并释放资源。
...
- `src/gesture`：摄像头生命周期、稳定手身份、21 点特征提取、Pinch／保持状态机、互斥动作路由、光标、教程与 Debug。模型推理限制为 20Hz，光标由渲染循环继续插值。
