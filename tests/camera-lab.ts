...
const report = () => {
  ...
  document.getElementById("camera-result")!.textContent =
    `样图 ${sampleName} | 镜像 ${mirrored} | 画面旋转 ${visualAngle >= 0 ? "+" : ""}${visualAngle}° | 阶段 ${d.stage} | 后端 ${d.backend ?? "无"} | 兼容 ${d.compatibility} | 推理帧 ${d.frames} | 原始手 ${d.rawHands} | 有效手 ${d.validHands} | 无手 ${Math.round(d.emptyForMs)}ms | 摄像头请求 ${requestedStreams} | ${s.tracking} | Gesture ${s.gesture} | Hand ${f.handedness} | Index Angle ${f.indexAngle === null ? "—" : `${f.indexAngle.toFixed(1)}°`} | Index Velocity ${f.indexAngularVelocity.toFixed(1)}°/s | Point Confidence ${f.pointConfidence.toFixed(3)} | Thumb Spread ${f.thumbSpread?.toFixed(3) ?? "—"} | Thumb Reach ${f.thumbReach.toFixed(3)} | Thumb Valid ${f.thumbGeometryValid} | Selection Phase ${f.selectionPhase} | Locked Target ${f.lockedTarget ? `${f.lockedTarget.kind}/${f.lockedTarget.id}` : "none"} | Selected ${s.selected ?? "none"} | Last Action ${f.lastAction} | Mode ${f.zoomMode} | Delta ${f.zoomDelta >= 0 ? "+" : ""}${f.zoomDelta.toFixed(1)}° | Speed ${f.zoomSpeed.toFixed(3)} | Scale ${f.zoomScale.toFixed(3)}${sampleError ? ` | ${sampleError}` : ""}`;
};
