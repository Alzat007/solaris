    ...
    <aside className="debug" aria-label="手势调试信息">
      ...
      <pre>
        {[
          ...
          `Action: ${f.action}`,
          `Selection Phase: ${f.selectionPhase}`,
          `Thumb Spread: ${f.thumbSpread === null ? "—" : f.thumbSpread.toFixed(3)}`,
          `Thumb Reach: ${f.thumbReach.toFixed(3)}`,
          `Thumb Geometry Valid: ${f.thumbGeometryValid}`,
          ...
          `Thumb Opening: ${(f.thumbOpenProgress * 100).toFixed(0)}%`,
          `Thumb Release Required: ${f.thumbNeedsRelease}`,
          `Index Angle (diagnostic): ${f.indexAngle === null ? "—" : `${f.indexAngle.toFixed(1)}°`}`,
          `Index Angular Velocity (diagnostic): ${f.indexAngularVelocity >= 0 ? "+" : ""}${f.indexAngularVelocity.toFixed(1)}°/s`,
          `Index State (diagnostic): ${f.indexState}`,
